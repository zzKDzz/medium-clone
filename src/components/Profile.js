...
class Profile extends Component{
  ...
  componentWillMount(){
    this.context.getUserProfile(this.props.match.params.id)
  }
  render(){
    return (
      <AppContext.Consumer>
        <div>
          {Object.keys(this.context.profile).length > 0 ? <ItemList items ={this.context} /> : ''}
        </div>
      </AppContext.Consumer>
    )
  }
              ...
              <div className="post-metadata">
                ...
                <div className="post-info">
                  <div data-react-className="PopoverLink">
                    <span className="popover-link" data-reactroot=""><a href="javascript:void(0);">{items.profile.user.name}</a></span>
                  </div>
                  <small>Published • a must read</small>
                </div>
              </div>
              {article.feature_img.length > 0 
                ? <div className="post-picture-wrapper"><img src={article.feature_img} alt="alt"/></div> 
                : ''
              }
